...
export function MediaMomentPlayer({ moment, className = '' }: { moment: MediaMomentType; className?: string }) {
  ...
  return (
    <div ref={containerRef} className={`relative overflow-hidden rounded-2xl bg-ink/10 ${className}`}>
      {everInView ? (
        <video
          ref={videoRef}
        ...
        <img src={moment.posterSrc} alt={moment.caption} loading="lazy" className="h-full w-full object-cover" />
      )}
      ...
      {everInView && playing && (
        <button
          ...
          className="absolute bottom-2.5 right-2.5 flex h-8 w-8 items-center justify-center rounded-full bg-ink/50 text-cream backdrop-blur-sm"
        >
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/85 to-transparent p-3 pt-8">
        <p className="text-sm leading-snug text-cream [text-shadow:0_1px_4px_rgba(0,0,0,0.5)]">{moment.caption}</p>
      </div>
    </div>
  )
}
