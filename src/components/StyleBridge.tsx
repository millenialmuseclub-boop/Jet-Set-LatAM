...
export function StyleBridge({destination,context,itinerary,carnival=false,commerce=true}: {destination:Destination;context?:TripContext;itinerary?:Itinerary;carnival?:boolean;commerce?:boolean}) {
 ...
 {offer&&<div className="mt-5 border-t border-ink/10 pt-4"><p className="eyebrow text-ink-soft">From the existing wardrobe edit</p><h3 className="mt-2 font-display text-xl">{offer.title}</h3><ul className="my-3 space-y-2 text-xs text-ink-soft">{offer.previewItems?.map(p=><li key={p}>{p}</li>)}</ul><button onClick={()=>openExternal(offer.affiliateUrl)} className="min-h-11 text-sm text-terracotta">{offer.cta} ↗</button><AffiliateDisclosure className="mt-3"/></div>}
 </section>
}
