...
import { getPlace, getGuide, getPlacePhoto } from "@/data";
import { Link } from "react-router-dom";

export function JetSetPickCard({ place }: { place: Place }) {
  const [saved, setSaved] = useState(() => isSavedPlace(place.id));
  const pairWith = place.pairWithPlaceId
    ? getPlace(place.pairWithPlaceId)
    : undefined;
  ...
  return (
    <article className="jet-set-pick min-w-[260px] max-w-[260px] snap-start rounded-2xl bg-cream shadow-sm ring-1 ring-ink/5">
      <div className="relative">
        ...
        />
        <span className="gold-glint absolute left-3 top-3 rounded-full bg-ink/70 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.12em] text-cream backdrop-blur-sm">
          Jet Set Pick
        </span>
        <button
          onClick={(e) => {
      ...
      <div className="p-4">
        ...
        <div className="flex items-baseline justify-between gap-2">
          ...
          {place.priceLevel && (
            <span className="text-xs text-gold-light font-medium bg-jungle-dark/90 rounded-full px-2 py-0.5 text-cream">
              {place.priceLevel}
            </span>
          )}
        </div>
        {place.neighborhood && (
          <p className="mt-0.5 text-xs uppercase tracking-[0.1em] text-terracotta">
            {place.neighborhood}
          </p>
        )}
        {place.pickDetails && (
          <div className="mt-3 space-y-1.5 text-sm text-ink-soft">
            <p>
              <span className="font-medium uppercase tracking-[0.06em] text-[11px] text-terracotta/80">
                Go for
              </span>
              <br />
              {place.pickDetails.goFor}
            </p>
            {pairWith && (
              <p>
                <span className="font-medium uppercase tracking-[0.06em] text-[11px] text-terracotta/80">
          ...
          <div className="-mr-1.5 flex items-center gap-0.5">
            {place.mapUrl && (
              <button
                ...
                className="flex items-center gap-1 rounded-full px-2 py-2 text-[11px] text-ink-soft/50 active:bg-ink/5 active:text-terracotta"
              >
                <Map size={12} /> Map
              </button>
            )}
